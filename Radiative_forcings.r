...
# Bind into a matrix
y <- as.data.table(cbind(y1, y2))

# time series
y_ts <- ts(y, start = min(merged_data$year), frequency = 1)  # annual data


# setnames(y, c("y1", "y2", "y3"), c("GMST", "ERF", "EEI"))
setnames(y, c("y1", "y2"), c("GMST", "ERF"))
plot(y_ts, main = "Climate Time Series")

### chosing r and p
lag_selection <- VARselect(y_ts, lag.max = 10, type = "const")
lag_selection$selection  # Recommended lag length

# Cointegration testing (Johansen)
# Choose K = selected lag + 1
selected_lag <- as.integer(lag_selection$selection["HQ(n)"])
johansen_test <- ca.jo(y, type = "trace", ecdet = "const", K = selected_lag)
summary(johansen_test)
####


p <- 2 # from varselect() function
lags <- p
...
cbind(y[-(1:lags),] ,na.exclude(lag.data.table(y, max.lag = 2)))
